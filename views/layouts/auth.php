<?php
/**
 * تخطيط صفحات المصادقة (تسجيل الدخول)
...
<head>
    ...
    <title><?= e($pageTitle ?? 'تسجيل الدخول') ?> | <?= e($systemName) ?></title>
...
<body class="auth-body">
    <div class="auth-wrap">
        <?php $logo = logo_data_uri(); ?>
        <div class="auth-brand">
            <?php if ($logo): ?>
                <img src="<?= $logo ?>" alt="الشعار" class="auth-logo">
            <?php else: ?>
                <div class="auth-logo-icon"><i class="bi bi-shield-check"></i></div>
            <?php endif; ?>
            <div>
                <h1 class="auth-brand-name"><?= e($systemName) ?></h1>
                <span class="auth-brand-sub">إدارة المستندات والتحقق عبر QR Code</span>
            </div>
        </div>
        ...
        <div class="auth-footer">
            <i class="bi bi-shield-lock"></i> نظام موثّق للتحقق من المستندات إلكترونيًا
        </div>
    </div>
